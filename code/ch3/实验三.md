...
| 指令    | 语法             | 说明                    | 转移范围                                |
| ------- | ---------------- | ----------------------- | --------------------------------------- |
| **JMP** | JMP 标号         | 无条件转移（段内短/近） | -128到+127（短）<br>-32768~+32767（近） |
...
ASCII中数字的编码是30H--39H，大写字母的编码是41H--5AH(其中A--F是41H-46H)，小写字母的编码是61H--7AH(61H--66h是a~f)，
...
③至于这个换行子程序只是调用了ASCII中回车0DH，换行0AH，其实如果ASCII记不住可以直接用引号包裹表示，如要显示1不写31H而写”1“，不过回车换行是不支持的。
